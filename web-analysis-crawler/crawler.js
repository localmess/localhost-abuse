...
const fs = require('fs');
const webRTCSrc = fs.readFileSync('./helpers/webRTCMonitor.js', 'utf8');
...
function openBrowser(log, proxyHost, executablePath) {
    ...
    const args = {
        args: [
            ...
            // "--enable-features=WebRtcAllowAllIPs"
        ]
    };
...
async function getSiteData(context, url, {
    ...
    if (runInEveryFrame) {
        ...
        if (CONSENT_WITH_PRIVACCEPT){
            await page.evaluateOnNewDocument(privAcceptSource);
        }
        
        if (INJECT_WEBRTC) {
            await page.evaluateOnNewDocument(webRTCSrc);
        }
    }

    for (let collector of collectors) {
        if(collector.id() === 'webRTC' && INJECT_WEBRTC) {
            try {
                // eslint-disable-next-line no-await-in-loop
    ...
    page.on('dialog', async dialog => {
        try {
          ...
          console.warn('Failed to dismiss dialog:', err.message);
        }
      });
    ...
    await page.waitForTimeout(COOKIE_WAIT_TIME);
    if (CONSENT_WITH_PRIVACCEPT) {
        await acceptAllCookies(page, log);
    }
    ...
    if (waitExtra) {
        await page.waitForTimeout(extraDelayForTestingMs);
    }
...
module.exports = async (url, options) => {
    ...
    try {
        ...
        if (browser && !VISUAL_DEBUG && HEADLESS) {
            await browser.close();
        }
    }
